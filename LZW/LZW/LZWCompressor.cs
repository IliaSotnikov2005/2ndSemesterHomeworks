...
public static class LZWCompressor
{
    ...
    public static void Compress(string inputPath)
    {
        ...
        {
            binaryWriter.Write(BitConverter.GetBytes(minBits));
    ...
    public static void Decompress(string inputPath)
    {
        ...
        File.WriteAllText(inputPath + ".unzipped", result);
    ...
    private static List<int> ReadCodesFromFile(string fileName)
    {
        ...
        int codeLength = BitConverter.ToInt32(bytes.Take(4).ToArray(), 0);
        var codes = new List<int>();

        int currentCode = 0;
        int currentCodeLength = 0;

        foreach (byte byteValue in bytes[4..])
        {
            for (int i = 0; i < 8; i++)
            {
